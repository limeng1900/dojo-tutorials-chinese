...
你可能会发现有需求数组中有一个名为`dojo/domReady!`的模块没有返回变量，它是一种加载器插件，用来控制加载器的行为。本模块的作用是让加载器等待DOM结构加载完成。在异步的世界里，操作假设的页面DOM结构可不是个好主意，所以如果你要在代码中对DOM做点事的话，先确定你包含了这个插件。因为我们在代码中并不使用这个插件，惯例是把它放在数组的最后，而且不提供它的返回变量。
